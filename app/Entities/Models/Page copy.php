...
{
    return $this->belongsToMany(\BookStack\Entities\Company::class, 'page_company_permissions')
        ->select('companies.id'); // Especificar a tabela para evitar ambiguidade
}

public function scopeVisibleToCompany($query, $companyId)
{
    return $query->where(function ($q) use ($companyId) {
        // Páginas sem restrições (abertas para todos)
        $q->whereDoesntHave('companies')
          // Ou páginas com permissão para a empresa específica
...
public function isAccessibleByCompany($companyId)
{
    // Se não tem empresas associadas, está aberta para todos
